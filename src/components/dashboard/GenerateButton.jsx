...
export default function GenerateButton({ profile, existingRecs = [], onGenerated }) {
  ...
  const prevCountRef = useRef(existingRecs.length);
  ...
  const generateRecommendations = async () => {
    if (!profile) return;
    setLoading(true);
    prevCountRef.current = existingRecs.length;
    toast.info('Generating suggestions — you can browse other pages!');

    // Fire and forget — backend handles everything
    ...
    // Poll for new recs regardless of which page user is on
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (fresh.length > prevCountRef.current) {
          ...
          setLoading(false);
          toast.success(`${fresh.length - prevCountRef.current} new suggestions ready! 🎉`);
          onGenerated?.();
        }
      } catch (e) {
        clearInterval(pollRef.current);
