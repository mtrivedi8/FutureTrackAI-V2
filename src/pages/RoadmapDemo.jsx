...
export default function RoadmapDemo() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-indigo-950 to-violet-950">
      ...
      {/* Main layout */}
      <div className="px-4 pb-32 pt-4 max-w-7xl mx-auto flex gap-6">
        {/* Tracks grid */}
        ...
        {/* Right panel: Special Programs & Opportunities (desktop only) */}
        <div className="hidden lg:block w-72 shrink-0">
          <OpportunitiesPanel selected={selected} navigate={navigate} />
        </div>
      </div>
        ...
        {selected && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setSelected(null)} />
            <motion.div
              initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-white/10 rounded-t-3xl p-5 z-50 max-h-[75vh] overflow-y-auto lg:hidden"
            >
